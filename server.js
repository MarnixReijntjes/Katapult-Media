...
// Optional: keep outbound stuff (unused unless you call /call-test)
...
if (!ELEVEN_API_KEY || !ELEVEN_VOICE_ID) {
  console.error('ElevenLabs env missing (ELEVENLABS_API_KEY / ELEVENLABS_VOICE_ID)');
  process.exit(1);
}

/* =======================
   TWILIO (optional outbound)
======================= */
...
const httpServer = http.createServer(async (req, res) => {
  ...
  // Outbound TwiML endpoint (optional)
...
wss.on('connection', (twilioWs) => {
  ...
  openaiWs.on('open', () => {
    // INBOUND instructions only (DES)
    const instructions = (INSTRUCTIONS_INBOUND || '').trim();
    ...
    console.log(`[${new Date().toISOString()}] OpenAI session.update sent (INBOUND DES)`);
  });
  ...
  twilioWs.on('message', (msg) => {
    ...
    if (data.event === 'start') {
      ...
      if (!greeted) {
        greeted = true;
        // INBOUND: greet immediately (no Move2Go)
        enqueueSpeak(INBOUND_OPENING);
      }
      return;
    }
  ...
  openaiWs.on('message', async (raw) => {
    ...
    if (evt.type === 'response.done') {
      ...
      if (assistantSpokenForTurn >= userTurn) return;
